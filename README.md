...
- 未标记已抢到的活动显示黄色报名书签；在活动列表手动标记“已抢到”后，移除该活动的黄色报名书签，改为显示红色活动时间书签。备忘录为绿色书签。
...
6. 在“活动汇总”点击“自动采集”选择间隔；抢到活动后点击该行“标记已抢到”。关闭主窗口后程序会留在系统托盘继续运行，右键托盘图标可退出。
...
其中 `last-results.json` 保存活动缓存，`timetable.json` 保存课表和备忘录，`settings.ini` 保存自动采集、桌面提醒及已抢到活动的标记和快照。重新采集只替换活动缓存；即使已抢到的活动从学校未开始列表中消失，红色活动提醒仍会保留。自动采集和电脑提醒要求程序保持运行；系统托盘不可用时关闭窗口会退出程序。
